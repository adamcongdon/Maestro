 ...
 * - open_file_tab: Open a file preview tab in a session
...
export class WebSocketMessageHandler {
	...
	private handleSelectSession(client: WebClient, message: WebClientMessage): void {
		...
		logger.info(
			`[Web] Calling selectSessionCallback for session ${sessionId}${tabId ? `, tab ${tabId}` : ''}${focus ? ', with focus' : ''}`,
			LOG_CONTEXT
		);
		this.callbacks
	...
	/**
	 * Handle open_file_tab message - open a file preview tab in a session
	 */
	...
	private handleRefreshFileTree(client: WebClient, message: WebClientMessage): void {
		const sessionId = message.sessionId as string;
		logger.info(
			`[Web] Received refresh_file_tree message: session=${sessionId}`,
			LOG_CONTEXT
		);
	...
	private handleRefreshAutoRunDocs(client: WebClient, message: WebClientMessage): void {
		const sessionId = message.sessionId as string;
		logger.info(
			`[Web] Received refresh_auto_run_docs message: session=${sessionId}`,
			LOG_CONTEXT
		);
	...
	/**
	 * Handle configure_auto_run message - configure and optionally launch an auto-run session
	 */
