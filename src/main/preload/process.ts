...
export function createProcessApi() {
	return {
		...
		/**
		 * Subscribe to remote open file tab from web/CLI interface
		 */
		onRemoteOpenFileTab: (
		...
		/**
		 * Subscribe to remote file tree refresh from web/CLI interface
		 */
		...
		/**
		 * Subscribe to remote auto-run docs refresh from web/CLI interface
		 */
		...
		/**
		 * Subscribe to remote auto-run configuration from web/CLI interface
		 * Includes response channel for sending result back to the main process
		 */
		onRemoteConfigureAutoRun: (
			...
			log('Registering onRemoteConfigureAutoRun listener');
			const handler = (
				_: unknown,
				sessionId: string,
				config: any,
				...
				log('Received remote:configureAutoRun IPC', { sessionId, responseChannel });
